...
function FilterGif({alignLeft=false,showTrending = false}) {
  ...
  return (
    <div className={`flex my-3 gap-3 ${alignLeft ? "":"justify-end"} ${showTrending ? "flex-col sm:flex-row sm:items-center justify-between":""}
    `}>
          ...
          <div className='flex sm:min-w-80 min-w-48 rounded-full bg-gray-800'>
            {
                filters.map((f)=>{
                    return <span key={f.title}
                    onClick={()=>setFilter(f.value)}
                    className={`${filter === f.value ? f.background: ""}
                    font-semibold py-2 w-1/3 rounded-full text-center cursor-pointer`}
                    >{f.title}</span>
                })
            }
          </div>
